...
// ════════════════════════════════════════════════════════
//  WHATSAPP RÉEL — Worker ESM (Baileys)
// ════════════════════════════════════════════════════════
const WA_WORKER_URL = 'http://localhost:3001';

async function callWAWorker(method, path, body) {
  try {
    const opts = { method, headers: { 'Content-Type': 'application/json' } };
    if (body) opts.body = JSON.stringify(body);
    const r = await fetch(WA_WORKER_URL + path, opts);
    return await r.json();
  } catch(e) {
    return { error: 'WhatsApp worker non disponible: ' + e.message };
  }
}

// Start WA worker as child process
const { spawn } = require('child_process');
const waWorker = spawn('node', [path.join(__dirname, 'wa-worker.mjs')], {
  env: { ...process.env, WA_PORT: '3001' },
  stdio: ['ignore', 'pipe', 'pipe']
});
waWorker.stdout.on('data', d => console.log('[WA]', d.toString().trim()));
waWorker.stderr.on('data', d => console.error('[WA ERR]', d.toString().trim()));
waWorker.on('close', code => { if (code !== 0) console.log('[WA Worker] exited:', code); });
console.log('[WA Worker] started PID:', waWorker.pid);
